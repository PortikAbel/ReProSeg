...
def get_patch_size(net: ReProSeg, args: argparse.Namespace) -> int:
    img_shape = DATASETS[args.dataset]["img_shape"]
    arbitrary_data = torch.zeros((img_shape[0], img_shape[1]), device=args.device)
    arbitrary_output = net(arbitrary_data)
    patch_size = arbitrary_data.shape[0] // arbitrary_output.shape[0]
    assert patch_size == arbitrary_data.shape[1] // arbitrary_output.shape[1]
    return patch_size
